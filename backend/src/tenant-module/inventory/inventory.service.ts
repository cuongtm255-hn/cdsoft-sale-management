...
import {
  ...
  StockReceiptPurchaseOrderOptionFilterDto,
  CreateStockIssueDto, UpdateStockIssueDto, StockIssueFilterDto, StockIssueSaleOrderOptionFilterDto,
} from './dto/inventory.dto';

const WAREHOUSE_TTL = 3600; // 1 hour
...
export class InventoryService {
  ...
  private get tenantCode() { return this.tenantCtx.getTenantCode()!; }
  ...
  private warehouseKey() { return `${this.tenantCode}:warehouses`; }

  // ─── Warehouses ──────────────────────────────────────────────────────────

  async listWarehouses() {
    return this.cache.getOrSet(this.warehouseKey(), WAREHOUSE_TTL, async () => {
  ...
  async createWarehouse(dto: CreateWarehouseDto) {
    const repo = await this.getRepo(Warehouse);
    const result = await repo.save(repo.create(dto));
    await this.cache.del(this.warehouseKey());
    return result;
  }
  ...
  async updateWarehouse(id: string, dto: UpdateWarehouseDto) {
    ...
    const result = await repo.save(wh);
    await this.cache.del(this.warehouseKey());
    return result;
  }
  ...
  async createReceipt(dto: CreateStockReceiptDto, userId: string): Promise<StockReceipt> {
    ...
    const purchaseOrder = dto.purchaseOrderId
      ? await this.resolvePurchaseOrderRef(ds, dto.purchaseOrderId, dto.warehouseId)
      : null;
    ...
    const receipt = repo.create({
      supplierId: purchaseOrder?.supplierId ?? dto.supplierId,
      purchaseOrderId: purchaseOrder?.id,
      warehouseId: dto.warehouseId,
      refCode: dto.refCode ?? purchaseOrder?.code,
  ...
  async getReceipts(filter: StockReceiptFilterDto) {
    ...
    const ds = await this.getDs();
    const purchaseOrderIds = [...new Set(data.map((receipt) => receipt.purchaseOrderId).filter(Boolean))];
    const orderRows = purchaseOrderIds.length
      ? await ds.query(
          `SELECT id, code FROM orders WHERE id IN (${purchaseOrderIds.map(() => '?').join(',')})`,
          purchaseOrderIds,
        )
      : [];
    const orderMap: Record<string, string> = Object.fromEntries(orderRows.map((row: any) => [row.id, row.code]));
    return {
      data: data.map((receipt) => ({
        ...receipt,
        purchaseOrderCode: receipt.purchaseOrderId ? orderMap[receipt.purchaseOrderId] ?? receipt.refCode : null,
      })),
      total,
      page: filter.page,
      limit: filter.limit,
  ...
  private async resolvePurchaseOrderRef(
    ds: DataSource,
    ...
    if (!normalizedRef) throw new BadRequestException('PURCHASE_ORDER_REQUIRED');

    const orders = await ds.query(
      `SELECT o.id, o.code, o.status, o.warehouse_id AS warehouseId, o.supplier_id AS supplierId
       FROM orders o
       WHERE o.deleted_at IS NULL
         AND o.type = ?
         AND (o.id = ? OR o.code = ?)
       LIMIT 1`,
      [OrderType.PURCHASE, normalizedRef, normalizedRef],
    );
    if (!orders.length) throw new BadRequestException('PURCHASE_ORDER_NOT_FOUND');
    ...
    if (order.warehouseId && order.warehouseId !== warehouseId) {
      throw new BadRequestException('PURCHASE_ORDER_WAREHOUSE_MISMATCH');
    }

    const duplicateParams: any[] = [ReceiptStatus.CANCELLED, order.id, order.code];
    let duplicateQuery = `
      SELECT id
      FROM stock_receipts
      WHERE deleted_at IS NULL
        AND status <> ?
        AND (purchase_order_id = ? OR ref_code = ?)
    `;
    if (currentReceiptId) {
      duplicateQuery += ' AND id <> ?';
      duplicateParams.push(currentReceiptId);
    }
    duplicateQuery += ' LIMIT 1';

    const duplicates = await ds.query(duplicateQuery, duplicateParams);
    if (duplicates.length) {
      throw new BadRequestException('PURCHASE_ORDER_ALREADY_ASSIGNED');
    }

    return order;
  }

  async listPurchaseOrdersForReceipt(filter: StockReceiptPurchaseOrderOptionFilterDto) {
    const ds = await this.getDs();
    ...
    const conditions = [
      ...
      `NOT EXISTS (
        ...
        WHERE sr.deleted_at IS NULL
          AND sr.status <> ?
          AND (sr.purchase_order_id = o.id OR sr.ref_code = o.code)
          ${filter.receiptId ? 'AND sr.id <> ?' : ''}
      )`,
    ];
    const params: any[] = [OrderType.PURCHASE, OrderStatus.CANCELLED, ReceiptStatus.CANCELLED];

    if (filter.receiptId) params.push(filter.receiptId);
    if (filter.warehouseId) {
      conditions.push('o.warehouse_id = ?');
      params.push(filter.warehouseId);
    }
    if (filter.search) {
      conditions.push('(o.code LIKE ? OR s.name LIKE ?)');
    ...
    const [rows, countRows] = await Promise.all([
      ds.query(
        `SELECT o.id, o.code, o.status, o.warehouse_id AS warehouseId, o.total_amount AS totalAmount,
  ...
  async getReceipt(id: string) {
    ...
    const [whRows, supplierRows, productRows, unitRows, purchaseOrderRows] = await Promise.all([
      ...
      receipt.purchaseOrderId
        ? ds.query('SELECT id, code FROM orders WHERE id = ? AND deleted_at IS NULL', [receipt.purchaseOrderId])
        : Promise.resolve([]),
    ]);
    ...
    return {
      ...
      purchaseOrderCode: purchaseOrderRows[0]?.code ?? null,
  ...
  async confirmReceipt(id: string, dto: ConfirmStockReceiptDto, userId: string) {
    ...
    const result = await this.getReceipt(id);

    if (result.purchaseOrderId && result.supplierId) {
      const dueDate = await this.invoicesService.resolveSupplierDueDate(result.supplierId, result.confirmedAt ?? new Date());
      await this.invoicesService.ensurePurchaseInvoice({
        supplierId: result.supplierId,
        purchaseOrderId: result.purchaseOrderId,
        stockReceiptId: result.id,
        totalAmount: Number(result.totalAmount ?? 0),
        notes: result.notes,
        dueDate,
        issuedAt: result.confirmedAt ?? new Date(),
  ...
  async updateReceipt(id: string, dto: UpdateStockReceiptDto) {
    ...
    const nextWarehouseId = dto.warehouseId ?? receipt.warehouseId;
    const nextPurchaseOrderRef = dto.purchaseOrderId !== undefined ? dto.purchaseOrderId : receipt.purchaseOrderId;
    const purchaseOrder = nextPurchaseOrderRef
      ? await this.resolvePurchaseOrderRef(ds, nextPurchaseOrderRef, nextWarehouseId, id)
      : null;

    if (dto.supplierId !== undefined || purchaseOrder) receipt.supplierId = purchaseOrder?.supplierId ?? dto.supplierId;
    if (dto.purchaseOrderId !== undefined || purchaseOrder) receipt.purchaseOrderId = purchaseOrder?.id;
    if (dto.warehouseId !== undefined) receipt.warehouseId = dto.warehouseId;
    if (dto.refCode !== undefined || purchaseOrder) receipt.refCode = dto.refCode ?? purchaseOrder?.code;
  ...
  private async resolveSaleOrderRef(
    ds: DataSource,
    orderRef: string | undefined,
    warehouseId: string,
    ...
    const normalizedRef = orderRef?.trim();
    if (!normalizedRef) {
      throw new BadRequestException('SALE_ORDER_REQUIRED');
    }

    const orders = await ds.query(
      `SELECT o.id, o.code, o.status, o.warehouse_id AS warehouseId
       FROM orders o
       WHERE o.deleted_at IS NULL
         AND o.type = ?
         AND (o.id = ? OR o.code = ?)
       LIMIT 1`,
      [OrderType.SALES, normalizedRef, normalizedRef],
    );

    if (!orders.length) {
      throw new BadRequestException('SALE_ORDER_NOT_FOUND');
    }

    const order = orders[0];
    if (order.warehouseId && order.warehouseId !== warehouseId) {
      throw new BadRequestException('SALE_ORDER_WAREHOUSE_MISMATCH');
    }
    if (order.status === OrderStatus.CANCELLED) {
      throw new BadRequestException('SALE_ORDER_CANCELLED');
    }

    const duplicateParams: any[] = [IssueType.SALE, IssueStatus.CANCELLED, order.id, order.code];
    let duplicateWhere = `
      SELECT id
      FROM stock_issues
      WHERE deleted_at IS NULL
    ...
    if (currentIssueId) {
      ...
      duplicateParams.push(currentIssueId);
    }
    duplicateWhere += ' LIMIT 1';

    const duplicates = await ds.query(duplicateWhere, duplicateParams);
    if (duplicates.length) {
      throw new BadRequestException('SALE_ORDER_ALREADY_ASSIGNED');
    }

    return order;
  }

  async listSaleOrdersForIssue(filter: StockIssueSaleOrderOptionFilterDto) {
    const ds = await this.getDs();
    const page = Number(filter.page ?? 1);
    const limit = Math.min(Number(filter.limit ?? 20), 100);
    const offset = (page - 1) * limit;
    const conditions = [
      ...
      `NOT EXISTS (
        ...
        WHERE si.deleted_at IS NULL
          AND si.issue_type = ?
          AND si.status <> ?
          AND (si.order_id = o.id OR si.order_id = o.code)
          ${filter.issueId ? 'AND si.id <> ?' : ''}
      )`,
    ];
    const params: any[] = [
      OrderType.SALES,
    ...
    if (filter.search) {
      conditions.push('(o.code LIKE ? OR c.name LIKE ?)');
      params.push(`%${filter.search}%`, `%${filter.search}%`);
    }
    ...
    const [rows, countRows] = await Promise.all([
      ds.query(
        `SELECT o.id, o.code, o.status, o.warehouse_id AS warehouseId,
                o.total_amount AS totalAmount, o.confirmed_at AS confirmedAt,
  ...
  async getIssues(filter: StockIssueFilterDto) {
    ...
    const orderRefs = [...new Set(issues.map((i) => i.orderId).filter(Boolean))];
    const [whRows, orderRows] = await Promise.all([
      whIds.length
        ? ds.query(`SELECT id, name FROM warehouses WHERE id IN (${whIds.map(() => '?').join(',')})`, whIds)
        : Promise.resolve([]),
      orderRefs.length
        ? ds.query(
            `SELECT o.id, o.code
             FROM orders o
             WHERE o.deleted_at IS NULL
               AND (o.id IN (${orderRefs.map(() => '?').join(',')}) OR o.code IN (${orderRefs.map(() => '?').join(',')}))`,
            [...orderRefs, ...orderRefs],
          )
        : Promise.resolve([]),
    ]);
    ...
    orderRows.forEach((order: any) => {
      orderMap[order.id] = order.code;
      orderMap[order.code] = order.code;
    });

    return {
      data: issues.map((i) => ({
        ...i,
        warehouseName: whMap[i.warehouseId],
        orderCode: i.orderId ? (orderMap[i.orderId] ?? i.orderId) : null,
      })),
      total,
      page: filter.page,
      limit: filter.limit,
  ...
  async getIssue(id: string) {
    ...
    const [whRows, productRows, unitRows, orderRows] = await Promise.all([
      ...
      issue.orderId
        ? ds.query(
            `SELECT o.id, o.code, c.name AS customerName
             FROM orders o
             LEFT JOIN customers c ON c.id = o.customer_id AND c.deleted_at IS NULL
             WHERE o.deleted_at IS NULL
               AND (o.id = ? OR o.code = ?)
    ...
    const productMap: Record<string, any> = Object.fromEntries(productRows.map((p: any) => [p.id, p]));
    const unitMap: Record<string, any> = Object.fromEntries(unitRows.map((u: any) => [u.id, u]));
    const resolvedOrder = orderRows[0];

    return {
      ...issue,
      orderId: resolvedOrder?.id ?? issue.orderId,
      orderCode: resolvedOrder?.code ?? issue.orderId ?? null,
      orderCustomerName: resolvedOrder?.customerName ?? null,
      warehouseName: whRows[0]?.name,
  ...
  async createIssueDraft(dto: CreateStockIssueDto, userId: string) {
    ...
    const saleOrder = dto.issueType === IssueType.SALE
      ? await this.resolveSaleOrderRef(ds, dto.orderId, dto.warehouseId)
      : null;

    const issue = issueRepo.create({
      warehouseId: dto.warehouseId, issueType: dto.issueType as IssueType,
      orderId: saleOrder?.id ?? dto.orderId, notes: dto.notes,
  ...
  async updateIssue(id: string, dto: UpdateStockIssueDto) {
    ...
    const nextWarehouseId = dto.warehouseId ?? issue.warehouseId;
    const nextIssueType = (dto.issueType ?? issue.issueType) as IssueType;
    const nextOrderRef = dto.orderId !== undefined ? dto.orderId : issue.orderId;
    const saleOrder = nextIssueType === IssueType.SALE
      ? await this.resolveSaleOrderRef(ds, nextOrderRef, nextWarehouseId, id)
      : null;
    ...
    if (nextIssueType === IssueType.SALE) {
      issue.orderId = saleOrder!.id;
    } else if (dto.orderId !== undefined || dto.issueType === IssueType.INTERNAL || dto.issueType === IssueType.DAMAGED) {
      issue.orderId = dto.orderId;
    }
  ...
  async confirmIssue(id: string, userId: string) {
    ...
    if (issue.issueType === IssueType.SALE) {
      const saleOrder = await this.resolveSaleOrderRef(ds, issue.orderId, issue.warehouseId, issue.id);
      if (issue.orderId !== saleOrder.id) {
        issue.orderId = saleOrder.id;
        await issueRepo.save(issue);
      }
    }
