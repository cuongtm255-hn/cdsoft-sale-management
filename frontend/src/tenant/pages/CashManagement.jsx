...
import {
  Button, Tabs, Space, Typography, Modal, Form, Input,
...
function FundsTab() {
  ...
  const handleCreateFund = async () => {
    try {
      ...
      message.error('Tạo quỹ thất bại');
    }
  };
      ...
      <Modal title="Tạo quỹ tiền mặt" open={fundModal} onOk={handleCreateFund} onCancel={() => setFundModal(false)} okText="Tạo">
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="balance" label="Số dư ban đầu">
            <CompactNumberInput wrapperStyle={{ width: '100%' }} style={{ width: '100%' }} min={0} formatGrouped suffix="₫" />
          </Form.Item>
        </Form>
      </Modal>
      ...
      <Modal title="Tạo phiếu thu / chi" open={modal} onOk={handleCreate} onCancel={() => { setModal(false); form.resetFields(); }} okText="Lưu" width={520}>
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="amount" label="Số tiền" rules={[{ required: true }]}>
            <CompactNumberInput wrapperStyle={{ width: '100%' }} style={{ width: '100%' }} min={0.01} formatGrouped suffix="₫" />
          </Form.Item>
