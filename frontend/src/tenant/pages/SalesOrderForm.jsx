...
import {
  Form, Select, Input, Button, Space, Divider, Row, Col, Typography, message, Modal, Tooltip,
} from 'antd';
...
export default function SalesOrderForm() {
  ...
  const handleOpenNewCustomer = () => {
    window.open('/tenant/customers/new', '_blank', 'noopener,noreferrer');
  };
  ...
  return (
    <div>
      <PageHeader title={t('orders.salesCreateTitle')} />
          ...
          <Col span={12}>
            <Form.Item
              ...
              label={
                <Space size={4}>
                  {t('orders.customer')}
                  <Tooltip title={t('orders.addNewCustomer')}>
                    <Button
                      type="link"
                      size="small"
            ...
            >
              <Select
                ...
                onDropdownVisibleChange={(open) => { if (open) reloadCustomers(); }}
                dropdownRender={(menu) => (
                  <>
                    {menu}
